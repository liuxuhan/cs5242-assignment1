...
"""
ID = 'e012345678' 
truth_path = 'code/b' #change truth_path = 'b' for verification
...
true_grads = list()
for f in true_file:
    true_grads.append(read_file(os.path.join(os.path.dirname(__file__),truth_path,f)))

score = list()
for i, fn in enumerate(file_name):
    # grads = read_file(os.path.join(ID,fn))
    grads = read_file(os.path.join(os.path.dirname(__file__),fn))
